-- marcar_apolice_ativa_por_nomes, nível 2 ("sem_conectores"): fazia
-- `limit 1` sem verificar quantos trabalhadores colapsam para o mesmo nome
-- depois de remover as palavras de ligação (de/dos/das/da/e) -- ao
-- contrário do nível 3 (semelhança), que só aceita candidato único. Se dois
-- trabalhadores diferentes colapsassem para o mesmo nome (ex: "Maria E
-- Costa" e "Maria Costa"), o match era escolhido arbitrariamente, sem o
-- aviso "⚠️ por semelhança" que marcarApoliceAtivaLote só gera para
-- metodo='semelhanca'. Aplica a mesma proteção do nível 3: só aceita o
-- nível 2 se houver exatamente um candidato.
...
begin
  foreach v_nome_bruto in array p_nomes loop
    ...
    -- 2. Sem palavras de ligação, dos dois lados — só aceite se for candidato único
