...
describe('calcMesParcial', () => {
  ...
  it('admissão a meio do mês (dia 17 de Maio 2026)', () => {
    ...
    // dias 17 a 30 inclusive, convenção 30 dias: 30 - 17 + 1 = 14
...
describe('calcSubsidiosAnoProportional', () => {
  ...
  it('admissão a 17 de Maio — fração correta do mês de admissão', () => {
    const r = calcSubsidiosAnoProportional(VENC, '2026-05-17', null, 2026);
    // Maio: 14 dias, Jun–Dez: 7 meses completos → frator = 14/30 + 7 = 7.4667
  ...
  it('cessação a 10 de Outubro — fração correta', () => {
    ...
    // Jan–Set: 9 meses, Out: 10 dias → frator = 9 + 10/30 = 9.333
    const esperado = (VENC / 12) * (9 + 10 / 30);
    expect(r.subsFeriasTotalAno).toBeCloseTo(esperado, 1);
  });

  it('admissão e cessação no mesmo mês (contrato curto)', () => {
    const r = calcSubsidiosAnoProportional(VENC, '2026-05-10', '2026-05-20', 2026);
    // 11 dias no mês → frator = 11/30
...
describe('calcAcertoCessacao', () => {
  ...
  it('5 dias de férias não gozadas → valor correto (vencBase/30 × 5)', () => {
  ...
  it('cessação a 10 de Outubro — subsídios proporcionais corretos', () => {
    ...
    // Jan–Set (9 meses) + 10 dias Out → frator = 9 + 10/30
    const esperado = (VENC / 12) * (9 + 10 / 30);
...
describe('calcDiasFeriasAnoAdmissao', () => {
  ...
  it('admitido a 17 de Maio → Junho a Dezembro são meses completos (7)', () => {
    // Maio é parcial (dia 17 a 30) → não conta como completo
    const r = calcDiasFeriasAnoAdmissao('2026-05-17', null, 2026);
    expect(r.mesesCompletos).toBe(7); // Jun, Jul, Ago, Set, Out, Nov, Dez
    expect(r.diasFerias).toBe(14);
  });

  it('contrato muito curto — cessação no mesmo mês da admissão', () => {
