...
import { FileText, FileSignature, FolderOpen, Mail, Building2, ReceiptText, Coins, Receipt, BarChart2, ArrowRightLeft, Landmark, ListChecks, AlertTriangle, Users } from 'lucide-react';
...
const DocumentsAdmin = ({ workers = [], documents = [], setDocuments, systemSettings, onSwitchTab, ...rest }) => {
  ...
  return (
    <div className="animate-in fade-in slide-in-from-bottom-4 duration-500">
      ...
      {/* Grupos (nível superior) — sublinhado laranja, mesmo padrão de Equipa */}
      <div className="flex flex-wrap items-end gap-1 mb-4 border-b border-slate-100 overflow-x-auto" style={{ scrollbarWidth: 'none' }}>
        {GROUPS.map(group => {
          ...
          return (
            <button
              ...
              className={`flex-shrink-0 flex items-center gap-1.5 px-3 pb-2.5 pt-1 text-[11px] font-black uppercase tracking-wider transition-all border-b-2 -mb-px whitespace-nowrap ${isActive ? 'border-[#EB8D00] text-[#1B3A57]' : 'border-transparent text-slate-400 hover:text-[#1B3A57]'}`}
            >
          ...
          {(() => {
            ...
            return expiring.length > 0 ? (
              <div className="flex items-center gap-3 bg-red-50 border border-red-200 rounded-xl px-4 py-3 mb-4">
                ...
                <button
                  onClick={() => { setValidadeFilter('expiring'); setDocMode('category'); }}
