...
import { FileText, FileSignature, FolderOpen, Mail, Building2, ReceiptText, Coins, Receipt, BarChart2, ArrowRightLeft, Landmark, ListChecks, AlertTriangle, LayoutGrid, List, Search, ScanSearch, Users } from 'lucide-react';
...
const DocumentsAdmin = ({ workers = [], documents = [], setDocuments, systemSettings, onSwitchTab, ...rest }) => {
  ...
  const GROUP_COLOR = {
    indigo:  { active: 'bg-[#1B3A57] text-white',    text: 'text-[#1B3A57]',    icon: 'bg-slate-100 text-[#1B3A57]' },
    blue:    { active: 'bg-[#869AAF] text-white',    text: 'text-[#869AAF]',    icon: 'bg-slate-100 text-[#869AAF]' },
    emerald: { active: 'bg-emerald-600 text-white',  text: 'text-emerald-600',  icon: 'bg-emerald-50 text-emerald-600' },
    violet:  { active: 'bg-[#EB8D00] text-white',    text: 'text-[#EB8D00]',    icon: 'bg-amber-50 text-[#EB8D00]' },
    sky:     { active: 'bg-slate-500 text-white',    text: 'text-slate-500',    icon: 'bg-slate-50 text-slate-500' },
  };
  ...
  return (
    <div className="bg-white rounded-2xl sm:rounded-[2.5rem] p-4 sm:p-6 lg:p-8 shadow-sm border border-slate-100 animate-in fade-in slide-in-from-bottom-4 duration-500">
        ...
        <>
          ...
          <div className="flex items-center gap-1 mb-4 border-b border-slate-100">
            {[
              ...
              <button
                key={id}
                onClick={() => setDocMode(id)}
                className={`flex items-center gap-1.5 px-3 pb-2.5 pt-1 text-[11px] font-black uppercase tracking-wider transition-all border-b-2 -mb-px ${docMode === id ? 'border-[#EB8D00] text-[#1B3A57]' : 'border-transparent text-slate-400 hover:text-[#1B3A57]'}`}
              >
                <Icon size={13} /> {label}
              </button>
            ))}
          ...
          {(() => {
            ...
            return expiring.length > 0 ? (
              <div className="flex items-center gap-3 bg-red-50 border border-red-200 rounded-xl px-4 py-3 mb-4">
                ...
                <button
                  onClick={() => { setValidadeFilter('expiring'); setViewMode('table'); setDocMode('category'); }}
            ...
            <div className="flex items-center justify-between mb-4">
              ...
              <div className="flex items-center bg-slate-100 p-1 rounded-xl gap-1">
                <button
                  ...
                  className={`flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-[10px] font-black uppercase transition-all ${viewMode === 'table' ? 'bg-white shadow-sm' : 'text-slate-400 hover:text-slate-600'}`}
                  style={viewMode === 'table' ? { color: '#1B3A57' } : {}}
                >
                  <List size={12} /> Lista
                </button>
                <button
                  onClick={() => setViewMode('folder')}
                  className={`flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-[10px] font-black uppercase transition-all ${viewMode === 'folder' ? 'bg-white shadow-sm' : 'text-slate-400 hover:text-slate-600'}`}
                  style={viewMode === 'folder' ? { color: '#1B3A57' } : {}}
                >
          ...
          {docMode === 'worker' && (<>
            <div className="flex gap-2 mb-5">
              ...
              <button
                onClick={() => setDocScannerOpen(true)}
                className="p-2.5 rounded-xl flex items-center justify-center transition-all border-2 shrink-0 hover:bg-slate-50"
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-3">
              {workers
                ...
                .map(w => {
                  const workerDocs = documents.filter(d => d.workerId === w.id);
                  const hasExpired = workerDocs.some(d => ['expirado', 'urgente'].includes(getValidadeStatus(d.data_validade)));
                  const initials = w.name
                    ? (() => { const p = w.name.trim().split(/\s+/); return p.length === 1 ? p[0][0].toUpperCase() : (p[0][0] + p[p.length - 1][0]).toUpperCase(); })()
                    : '?';
                  return (
                    <button
                      key={w.id}
                      onClick={() => { setSearchTerm(w.name); setDocMode('category'); setViewMode('table'); }}
                      className="bg-white border border-slate-100 hover:border-slate-200 hover:shadow-md rounded-2xl p-4 text-center transition-all"
                    >
                      <div className="flex flex-col items-center gap-2">
                        <div className="w-12 h-12 rounded-full flex items-center justify-center text-sm font-black" style={{ backgroundColor: '#1B3A57', color: '#EB8D00' }}>{initials}</div>
                        <div>
                          <p className="font-black text-slate-800 text-xs truncate max-w-[100px]">{w.name}</p>
                          <p className="text-[10px] text-slate-400">{workerDocs.length} doc{workerDocs.length !== 1 ? 's' : ''}</p>
                        </div>
                        {workerDocs.length > 0 && (
                          <div className={`w-2 h-2 rounded-full ${hasExpired ? 'bg-rose-500' : 'bg-emerald-500'}`} />
                        )}
                      </div>
                    </button>
                  );
      ...
      {docScannerOpen && <DocumentScannerModal open={docScannerOpen} onClose={() => setDocScannerOpen(false)} />}
