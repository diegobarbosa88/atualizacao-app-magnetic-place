...
// Consolida os antigos badges de regra (um por tipo, mutuamente exclusivos
// porque dependem todos de item.rule) numa única etiqueta — mesma informação,
// um só elemento em vez de nove condicionais empilhados.
function getRuleTag(item) {
  if (item.rule === 'internal_transfer_confirmed') return { label: 'interna · confirmada', cls: 'bg-cyan-100 text-cyan-700' };
  if (item.rule === 'internal_transfer_presumed' && item.ambiguous) return { label: 'interna · ambígua, revê', cls: 'bg-amber-100 text-amber-700' };
  if (item.rule === 'internal_transfer_presumed') return { label: 'interna · presumida', cls: 'bg-cyan-100 text-cyan-700' };
  if (item.rule === 'confirmed_manual') return { label: 'confirmado', cls: 'bg-indigo-100 text-indigo-600' };
  if (item.rule === 'alias_match') return { label: 'alias', cls: 'bg-violet-100 text-violet-600' };
  if (item.rule === 'manual') return { label: 'manual', cls: 'bg-indigo-100 text-indigo-600' };
...
export default function ResultadosTabs({
  displayData, activeRun, lastCreatedRun, voltarAoRunAtual, saldoManual,
  ...
  return (
    <div className="bg-white rounded-[2.5rem] shadow-sm border border-[var(--border-soft)] p-4 sm:p-8">
      ...
      {activeSubTab === 'matched' && (() => {
        ...
        const renderMatchedItem = (item, i) => {
          const isInterno = item.rule?.startsWith('internal_transfer');
        ...
        const itemsWithIdx = items.map((item, i) => ({ item, i }));
        const internalEntries = itemsWithIdx.filter(({ item }) => item.rule?.startsWith('internal_transfer'));
        const normalEntries = itemsWithIdx.filter(({ item }) => !item.rule?.startsWith('internal_transfer'));
        const nConfirmadas = internalEntries.filter(({ item }) => item.rule === 'internal_transfer_confirmed').length;
        const nPresumidas = internalEntries.filter(({ item }) => item.rule === 'internal_transfer_presumed' && !item.ambiguous).length;
        const nAmbiguas = internalEntries.filter(({ item }) => item.rule === 'internal_transfer_presumed' && item.ambiguous).length;
              ...
              <div className="flex items-center justify-between mb-1">
                ...
                {selOrphan.size > 0 && (
                  <button onClick={() => pedirObservacaoOrphan([...selOrphan])}
                    className="flex items-center gap-1 bg-[var(--orange)] text-[var(--navy-solid)] rounded-xl px-4 py-1.5 text-[10px] font-black uppercase tracking-widest hover:bg-[var(--orange-hover)] transition-all">
                  ...
                  <div className="flex items-start gap-3 min-w-0 flex-1">
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      {item.reason === 'ambiguous' && item.candidates && (() => {
                        const counts = {};
                        item.candidates.forEach(c => { if (c.entidade) counts[c.entidade] = (counts[c.entidade] || 0) + 1; });
                        const labels = Object.entries(counts).map(([nome, n]) => n > 1 ? `${nome} (${n} faturas)` : nome);
                        return (
                          <p className="text-[10px] text-amber-600 mt-1">Candidatos: {labels.join(', ')}</p>
                        );
                      })()}
