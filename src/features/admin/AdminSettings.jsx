  ...
  return (
    <button
      ...
      className={`relative text-left rounded-2xl border-2 p-3 transition-all ${
        selected
          ? 'shadow-sm'
          : 'border-[var(--border)] bg-white hover:border-[var(--border)]'
      }`}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4 sm:gap-6 lg:gap-8">
        ...
        <div className="bg-white p-4 sm:p-6 lg:p-8 rounded-2xl sm:rounded-[2.5rem] shadow-sm border border-[var(--border-soft)] lg:col-span-2">
          <div className="flex items-center justify-between mb-6">
            <div className="flex items-center gap-3">
              ...
              <h3 className="font-black text-lg text-[var(--ink)]">Administradores</h3>
            </div>
          ...
          <div className="space-y-2 mb-4">
            {workers.filter(w => w.isAdmin).length === 0 && !adminFormMode && (
              <p className="text-xs text-[var(--slate-dim)] font-bold text-center py-4">Nenhum administrador configurado.</p>
            )}
            {workers.filter(w => w.isAdmin).map(w => {
              ...
              return (
                <div key={w.id} className={`p-3 bg-[var(--surface)] rounded-2xl border transition-all ${isEditing ? 'border-indigo-200 bg-indigo-50/30' : 'border-[var(--border-soft)]'}`}>
                  {isEditing ? (
                    <div className="space-y-2">
                      <input type="text" value={adminForm.name} onChange={e => setAdminForm(p => ({ ...p, name: e.target.value }))} placeholder="Nome completo" className="w-full bg-white border border-[var(--border)] rounded-xl p-2.5 text-sm outline-none focus:ring-2 focus:ring-[var(--navy)]" />
          ...
          {(adminFormMode === 'new' || adminFormMode === 'existing') && (
            <div className="border-t border-[var(--border-soft)] pt-5 space-y-3">
              <p className="text-xs font-black text-[var(--slate-dim)] uppercase tracking-widest">Novo Administrador</p>
              {nonAdminWorkers.length > 0 && (
                <select
                  value={adminForm.selectedWorkerId}
                  onChange={e => handleSelectExistingWorker(e.target.value)}
                  className="w-full bg-[var(--surface)] border border-[var(--border)] rounded-xl p-3 text-sm outline-none focus:ring-2 focus:ring-[var(--navy)]"
                >
                  <option value="">— Selecionar trabalhador existente —</option>
                  {nonAdminWorkers.map(w => <option key={w.id} value={w.id}>{w.name}</option>)}
                </select>
              )}
              <input type="text" placeholder="Nome completo" value={adminForm.name} onChange={e => setAdminForm(p => ({ ...p, name: e.target.value }))} className="w-full bg-[var(--surface)] border border-[var(--border)] rounded-xl p-3 text-sm outline-none focus:ring-2 focus:ring-[var(--navy)]" />
        ...
        <div className="bg-white p-4 sm:p-6 lg:p-8 rounded-2xl sm:rounded-[2.5rem] shadow-sm border border-[var(--border-soft)]">
          <div className="flex items-center gap-3 mb-6">
            ...
            <h3 className="font-black text-lg text-[var(--ink)]">Segurança da Conta</h3>
          </div>
          <div className="space-y-4">
            <p className="text-xs text-[var(--slate-dim)] font-bold uppercase tracking-widest">Alterar Senha Administrador</p>
            <div className="flex gap-2">
              <input type="password" placeholder="Nova Senha" className="flex-1 bg-[var(--surface)] border border-[var(--border)] rounded-xl p-3 text-sm outline-none" id="new-admin-pass" />
        ...
        <div className="bg-white p-4 sm:p-6 lg:p-8 rounded-2xl sm:rounded-[2.5rem] shadow-sm border border-[var(--border-soft)]">
          <div className="flex items-center gap-3 mb-4">
            ...
            <div>
              <h3 className="font-black text-lg text-[var(--ink)]">Segurança Social — Comunicações</h3>
              <p className="text-xs text-[var(--slate-dim)] font-medium">Plataforma de Serviços de Interoperabilidade (PSI)</p>
            </div>
          </div>
          ...
          <div className="space-y-3">
            {ssStatus === null ? (
              <div className="flex items-center gap-2 text-[var(--slate-dim)] text-xs"><Loader2 size={13} className="animate-spin" /> A verificar configuração…</div>
            ) : (
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
                ...
                <div className={`flex items-center gap-2 px-3 py-2.5 rounded-xl text-xs font-bold border ${ssStatus.ambiente === 'producao' ? 'bg-[var(--surface)] border-[var(--border)] text-[var(--ink-mid)]' : 'bg-orange-50 border-orange-200 text-orange-700'}`}>
                ...
                {ssStatus.nissEmpresa && (
                  <div className="flex items-center gap-2 px-3 py-2.5 rounded-xl text-xs font-bold border bg-[var(--surface)] border-[var(--border)] text-[var(--slate-dim)]">
            ...
            <div className="border border-[var(--border-soft)] rounded-xl overflow-hidden">
              <button
                ...
                className="w-full flex items-center justify-between px-4 py-3 bg-[var(--surface)] hover:bg-[var(--surface-dim)] transition-colors"
              >
                <span className="text-xs font-black text-[var(--ink-mid)] uppercase tracking-wide">Como aderir à PSI (pré-requisito)</span>
                {ssPsiGuideOpen ? <ChevronUp size={14} className="text-[var(--slate)]" /> : <ChevronDown size={14} className="text-[var(--slate)]" />}
              </button>
              {ssPsiGuideOpen && (
          ...
          <div className="bg-white p-4 sm:p-6 lg:p-8 rounded-2xl sm:rounded-[2.5rem] shadow-sm border border-[var(--border-soft)]">
            <div className="flex items-center gap-3 mb-5">
              ...
              <div>
                ...
                <p className="text-xs text-[var(--slate-dim)] font-medium">Comprovativos · Documentos · Remunerações Permanentes</p>
              </div>
            </div>
            <SSConsultasPanel />
          </div>
        ...
        <div className="bg-white p-4 sm:p-6 lg:p-8 rounded-2xl sm:rounded-[2.5rem] shadow-sm border border-[var(--border-soft)]">
          <div className="flex items-start justify-between gap-4">
            <div className="flex items-center gap-3">
              ...
              <div>
                <h3 className="font-black text-lg text-[var(--ink)]">Importar Contratos da SS Direta</h3>
                <p className="text-xs text-[var(--slate-dim)] font-medium mt-0.5">Sincronizar vínculos exportados da Segurança Social Direta com os perfis dos trabalhadores</p>
              </div>
            </div>
            <button
          ...
          <div className="mt-4 text-xs text-[var(--slate-dim)] space-y-1 pl-11">
            <p>1. Na SS Direta: <span className="font-semibold text-[var(--slate-dim)]">Emprego → Vínculos de trabalhadores → Exportar</span></p>
        ...
        <div className="bg-white p-4 sm:p-6 lg:p-8 rounded-2xl sm:rounded-[2.5rem] shadow-sm border border-[var(--border-soft)] lg:col-span-2">
          <div className="flex items-center gap-3 mb-6">
            ...
            <h3 className="font-black text-lg text-[var(--ink)]">Inteligência Artificial — Gemini</h3>
          </div>
        ...
        <div className="bg-white p-4 sm:p-6 lg:p-8 rounded-2xl sm:rounded-[2.5rem] shadow-sm border border-[var(--border-soft)]">
          <div className="flex items-center gap-3 mb-6">
            ...
            <h3 className="font-black text-lg text-[var(--ink)]">Identidade da Empresa</h3>
          </div>
          <div className="space-y-4">
            <p className="text-xs text-[var(--slate-dim)] font-bold uppercase tracking-widest">Nome da Empresa</p>
            <input
              type="text"
              value={systemSettings.companyName}
              onChange={(e) => setSystemSettings(prev => ({ ...prev, companyName: e.target.value }))}
              onBlur={(e) => updateSetting('companyName', e.target.value)}
              className="w-full bg-[var(--surface)] border border-[var(--border)] rounded-xl p-3 text-sm focus:ring-2 focus:ring-[var(--navy)] outline-none font-bold"
            />
            <p className="text-xs text-[var(--slate-dim)] font-bold uppercase tracking-widest">Morada</p>
            <input
              type="text"
              ...
              className="w-full bg-[var(--surface)] border border-[var(--border)] rounded-xl p-3 text-sm focus:ring-2 focus:ring-[var(--navy)] outline-none font-bold"
            />
            <p className="text-xs text-[var(--slate-dim)] font-bold uppercase tracking-widest">NIF</p>
            <input
              type="text"
        ...
        <div className="bg-white p-4 sm:p-6 lg:p-8 rounded-2xl sm:rounded-[2.5rem] shadow-sm border border-[var(--border-soft)]">
          <div className="flex items-center gap-3 mb-6">
            ...
            <h3 className="font-black text-lg text-[var(--ink)]">Visual e Tema</h3>
          </div>
          <div className="space-y-6">
            ...
            <div className="flex items-center justify-between p-4 bg-[var(--surface)] rounded-2xl border border-[var(--border-soft)]">
              ...
              <button
                ...
                className={`w-14 h-8 rounded-full transition-all flex items-center px-1 ${systemSettings.darkMode ? '' : 'bg-[var(--border)]'}`}
            ...
            <div className="flex items-center justify-between p-4 bg-[var(--surface)] rounded-2xl border border-[var(--border-soft)]">
              ...
              <select
                ...
                className="bg-white border border-[var(--border)] rounded-xl px-3 py-2 text-sm font-bold shadow-sm"
              >
        ...
        <div className="bg-white p-4 sm:p-6 lg:p-8 rounded-2xl sm:rounded-[2.5rem] shadow-sm border border-[var(--border-soft)]">
          <div className="flex items-center gap-3 mb-6">
            ...
            <h3 className="font-black text-lg text-[var(--ink)]">Avisos de Falta</h3>
          </div>
          <div className="space-y-5">
            ...
            <div className="flex items-center justify-between p-4 bg-[var(--surface)] rounded-2xl border border-[var(--border-soft)]">
              ...
              <button
                ...
                className={`relative w-11 h-6 rounded-full transition-colors ${absenceConfig.absence_notify_client ? 'bg-orange-500' : 'bg-[var(--border)]'}`}
              >
            ...
            {/* Reasons list */}
            <div>
              <p className="text-xs font-black uppercase text-[var(--slate-dim)] tracking-widest mb-3">Motivos Predefinidos</p>
              <div className="space-y-2">
                {(absenceConfig.absence_reasons || []).map((r, i) => (
                  <div key={i} className="flex items-center gap-2 bg-[var(--surface)] border border-[var(--border)] rounded-xl px-3 py-2">
                    <span className="flex-1 text-sm font-bold text-[var(--ink-mid)]">{r}</span>
                    {(absenceConfig.absence_reasons || []).length > 1 && (
                      <button
                        ...
                        className="p-1.5 text-[var(--slate)] hover:text-rose-500 transition-colors rounded-lg"
                      >
                        <Trash2 size={13} />
                      </button>
                    )}
              ...
              <div className="flex gap-2 mt-3">
                <input
                  ...
                  className="flex-1 bg-[var(--surface)] border border-[var(--border)] rounded-xl px-3 py-2 text-sm font-bold outline-none focus:border-orange-400 focus:ring-2 focus:ring-orange-100 transition-all"
                />
                <button
                  onClick={() => {
      ...
      {showRecalcModal && (
        <ModalShell
          ...
          footer={
            <div className="flex gap-2 p-6">
              {!recalcProgress.done ? (
                <>
                  <button onClick={() => setShowRecalcModal(false)} className="flex-1 px-4 py-2.5 bg-[var(--border)] text-[var(--ink-mid)] rounded-xl text-xs font-black uppercase tracking-wider hover:bg-[var(--border)] transition-colors">
                    Cancelar
                  </button>
        ...
        >
          <div className="p-6">
            ...
            {recalcProgress.total > 0 && !recalcProgress.done && (
              <div className="mb-4">
                <div className="flex justify-between text-xs font-bold text-[var(--ink-soft)] mb-2">
