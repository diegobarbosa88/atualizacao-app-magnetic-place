...
export default function InternoModal({ tx, onClose, onSave }) {
  ...
  const handleConfirm = async () => {
    setSaving(true);
    await onSave(tx, { saveAlias, aliasName });
    setSaving(false);
    onClose();
  };
      ...
      <div className="bg-white rounded-3xl shadow-2xl w-full max-w-sm p-6 space-y-4">
        ...
        <div className="flex gap-2 pt-1">
          ...
          <button disabled={saving} onClick={handleConfirm}
            ...
            {saving ? <Loader2 size={13} className="animate-spin" /> : <ArrowLeftRight size={13} />} Confirmar
          </button>
        </div>
      </div>
    </div>
