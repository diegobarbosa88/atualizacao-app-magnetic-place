...
function AuditRow({ entry }) {
  ...
  const meta = ACTION_LABELS[entry.action] || { label: entry.action, color: 'bg-[var(--surface-dim)] text-[var(--ink-soft)] border-[var(--border)]' };
    ...
    <div className="border-b border-[var(--border-soft)] last:border-0">
      ...
      {expanded && (
        <div className="px-10 pb-3 space-y-1.5">
          {entry.note && <p className="text-xs text-[var(--slate-dim)] italic border-l-2 border-[var(--border)] pl-2">{entry.note}</p>}
  ...
  return (
    <div className="bg-white rounded-2xl border border-[var(--border-soft)] overflow-hidden">
      ...
      >
        ...
        <ChevronDown size={14} className={`text-[var(--slate)] shrink-0 transition-transform duration-200 ${expanded ? 'rotate-180' : ''}`} />
      </button>
    ...
    <div className="space-y-4">
      ...
      {loadError ? (
        ...
        <div className="flex items-center justify-center py-16">
          <div className="w-8 h-8 border-4 border-[var(--border)] rounded-full animate-spin" style={{ borderTopColor: FT.navy }} />
        </div>
      ) : grouped.length === 0 ? (
        <div className="bg-white rounded-2xl border border-[var(--border-soft)] p-12 text-center">
          <Shield size={24} className="mx-auto text-[var(--slate)] mb-3" />
          <p className="font-black text-[var(--slate-dim)] text-sm uppercase tracking-widest">Sem registos de auditoria</p>
          <p className="text-xs text-[var(--slate-dim)] mt-1">As ações dos clientes no portal aparecem aqui</p>
        </div>
      ) : (
