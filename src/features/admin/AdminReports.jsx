...
import {
  FileText, History, Users, Building2, Activity, Zap, X
} from 'lucide-react'
import ClientTimesheetReport from '../../components/common/ClientTimesheetReport'

export default function AdminReports() {
  const {
    workers, clients, logs, clientApprovals,
  ...
  const derivedReportData = useMemo(() => {
  ...
  return (
    <div className="animate-in fade-in slide-in-from-bottom-4 duration-500 space-y-4 sm:space-y-6">
      ...
      {printingReport && (
        <div
          className="fixed inset-0 bg-slate-900/80 backdrop-blur-lg z-[200] flex items-start justify-center p-4 overflow-y-auto"
        ...
        >
          <div className="w-full max-w-2xl lg:max-w-5xl mx-4 sm:mx-auto my-8 bg-white rounded-[2rem] sm:rounded-[3rem] shadow-2xl border border-indigo-100 animate-in fade-in zoom-in duration-300">
            ...
            <ClientTimesheetReport data={derivedReportData} onBack={() => setPrintingReport(null)} isEmbedded={true} />
          </div>
        </div>
      )}
