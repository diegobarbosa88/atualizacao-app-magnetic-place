...
function TypeBadge({ tipo }) {
  ...
  return <span className="px-2 py-0.5 rounded-md text-[9px] font-black uppercase bg-[var(--surface-dim)] text-[var(--slate-dim)]">Sem tipo</span>;
}
  ...
  return (
    <div>
      ...
      {isAddingExpense && (
        <div className="mb-6 bg-white p-4 sm:p-6 rounded-2xl shadow-inner border border-[var(--border-soft)]">
          ...
          <div className="mt-6 flex items-center gap-3">
            ...
            <button onClick={() => { setIsAddingExpense(false); setExpenseForm({ id: null, name: '', amount: '', type: 'fixo', date: toISODateLocal(new Date()) }); }} className="px-6 py-5 bg-[var(--surface-dim)] text-[var(--ink-soft)] rounded-2xl font-black text-sm uppercase shadow-sm hover:bg-[var(--border)] transition-colors">Cancelar</button>
          </div>
        </div>
      )}
      ...
      <div className="flex items-start gap-2.5 bg-white border border-[var(--border)] rounded-2xl px-4 py-3 mb-5 text-xs text-[var(--slate-dim)]" style={{ borderLeftWidth: '3px', borderLeftColor: FT.slate }}>
        ...
        <p>Mês definido por <strong className="text-[var(--ink-mid)] font-black">data de pagamento</strong> quando disponível, senão <strong className="text-[var(--ink-mid)] font-black">data da fatura</strong>. Faturas sem nenhuma das duas aparecem em <strong className="text-[var(--ink-mid)] font-black">Faturas sem data</strong> — nunca atribuídas silenciosamente a um mês.</p>
      </div>
        ...
        <table className="w-full text-left border-separate border-spacing-y-2">
          ...
          <tbody>
            {allExpensesSorted.length === 0 ? (
              <tr><td colSpan="5" className="py-16 text-center text-[var(--slate-dim)] text-sm font-medium">Sem despesas para o período selecionado.</td></tr>
            ) : allExpensesSorted.map((exp) => (
              <tr key={exp.id} className="bg-[var(--surface)] hover:bg-white hover:shadow-md transition-all duration-300">
                <td className="px-4 py-3 rounded-l-2xl border-y border-l border-[var(--border-soft)] text-xs font-bold text-[var(--slate-dim)] whitespace-nowrap font-mono">
                  {new Date(exp.date).toLocaleDateString('pt-PT', { day: '2-digit', month: '2-digit', year: '2-digit' })}
                </td>
                <td className="px-4 py-3 border-y border-[var(--border-soft)] text-sm font-bold text-[var(--ink)]">{exp.name}</td>
                <td className="px-4 py-3 border-y border-[var(--border-soft)]">
                  {exp._isFatura ? (
                ...
                <td className="px-4 py-3 rounded-r-2xl border-y border-r border-[var(--border-soft)] text-right">
                  {exp._isFatura ? (
            ...
            <table className="w-full text-left border-separate border-spacing-y-1">
              ...
              <tbody>
                {faturasClienteExcluidas.map(f => {
                  ...
                  return (
                    <tr key={f.id}>
                      <td className="px-4 py-2 rounded-l-xl border-y border-l border-[var(--border-soft)] text-sm text-[var(--ink-mid)] font-bold">{f.dados?.fornecedor || f.entidade || '—'}</td>
                      <td className="px-4 py-2 border-y border-[var(--border-soft)]"><TypeBadge tipo="cliente" /></td>
                      <td className="px-4 py-2 border-y border-[var(--border-soft)] text-xs font-bold text-[var(--slate-dim)] font-mono whitespace-nowrap">
                        {data_ref ? new Date(data_ref).toLocaleDateString('pt-PT', { day: '2-digit', month: '2-digit', year: '2-digit' }) : '—'}
                      </td>
                      <td className="px-4 py-2 rounded-r-xl border-y border-r border-[var(--border-soft)] text-sm font-black text-emerald-600 text-right whitespace-nowrap">{formatCurrency(parseFaturaValor(f))}</td>
                    </tr>
                  );
                })}
              </tbody>
              ...
              <table className="w-full text-left border-separate border-spacing-y-1">
                ...
                <tbody>
                  {faturasSemData.map(f => (
                    <tr key={f.id}>
                      <td className="px-4 py-2 rounded-l-xl border-y border-l border-[var(--border-soft)] text-sm text-[var(--ink-soft)]">{f.dados?.fornecedor || f.entidade || f.filename || '—'}</td>
                      <td className="px-4 py-2 border-y border-[var(--border-soft)]"><TypeBadge tipo={f.tipo || 'indefinido'} /></td>
                      <td className="px-4 py-2 rounded-r-xl border-y border-r border-[var(--border-soft)] text-sm font-bold text-[var(--slate-dim)] text-right whitespace-nowrap">{formatCurrency(parseFaturaValor(f))}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
                  ...
                  {faturasExcluidas.map(f => {
                    ...
                    return (
                      <tr key={f.id} className="opacity-50 hover:opacity-80 transition-opacity">
                        <td className="px-4 py-2 rounded-l-xl border-y border-l border-[var(--border-soft)] text-xs font-bold text-[var(--slate-dim)] font-mono whitespace-nowrap">
                          {data_ref ? new Date(data_ref).toLocaleDateString('pt-PT', { day: '2-digit', month: '2-digit', year: '2-digit' }) : '—'}
                        </td>
