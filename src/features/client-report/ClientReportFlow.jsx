...
const ClientReportFlow = ({ clientId, month, workers, logs, onClose, initialStep }) => {
  ...
  if (step === 'success') {
    return (
      <div className="max-w-xl mx-auto py-16 text-center animate-fade-in">
        ...
        <button onClick={() => { setStep('home'); onClose?.(); }} className={`px-6 py-3 bg-slate-900 text-white rounded-xl ${SCALE.text.badge}`}>Voltar ao Portal</button>
      </div>
    );
  }
  ...
  if (step === 'mode') return (
    <div className="max-w-2xl mx-auto py-6 animate-fade-in">
      <button onClick={() => initialStep === 'mode' ? onClose?.() : setStep('home')} className={`flex items-center gap-2 text-slate-400 hover:text-slate-700 ${SCALE.text.badge} mb-6`}>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
        <button onClick={() => setStep('quick')} className="text-left p-6 bg-white border-2 border-indigo-200 hover:border-indigo-400 rounded-2xl transition-all group">
          ...
          <div className={`mt-4 flex items-center gap-1 text-indigo-600 ${SCALE.text.badge}`}>Escolher este <ChevronDown size={12} className="-rotate-90" /></div>
        </button>
        <button onClick={() => setStep('precision')} className="text-left p-6 bg-white border-2 border-amber-200 hover:border-amber-400 rounded-2xl transition-all group">
          ...
          <div className={`mt-4 flex items-center gap-1 text-amber-600 ${SCALE.text.badge}`}>Escolher este <ChevronDown size={12} className="-rotate-90" /></div>
        </button>
      </div>
    </div>
  );
    ...
    <div className="max-w-3xl mx-auto py-6 animate-fade-in">
      <header className="bg-white rounded-3xl border border-slate-100 p-6 mb-6 flex items-center justify-between flex-wrap gap-4">
        ...
        <button
          ...
          className={`px-5 py-3 bg-indigo-600 hover:bg-indigo-700 text-white rounded-xl ${SCALE.text.badge} disabled:opacity-40 disabled:cursor-not-allowed flex items-center gap-2`}
        >
          <AlertCircle size={14} /> Reportar Divergência
        </button>
      </header>
      <section>
        <h3 className={`${SCALE.text.badge} text-slate-400 mb-3`}>Histórico</h3>
      ...
      {onClose && (
        <div className="mt-8 text-center">
          <button onClick={onClose} className={`text-slate-400 hover:text-slate-700 ${SCALE.text.badge}`}>Voltar ao Portal</button>
        </div>
      )}
    </div>
  );
