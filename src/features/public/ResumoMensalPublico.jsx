...
import {
  calcularRecibo, getIRSTabelasPorAno, getRateAtDate as _getRateAtDate,
  MESES_PT, eur,
...
function parseMes(str) {
  ...
  return { ano: parseInt(ano) || new Date().getFullYear(), mes: parseInt(mes) || new Date().getMonth() + 1 };
}
...
export default function ResumoMensalPublico() {
  ...
  const inicial = params.get('mes') || mesStr(new Date().getFullYear(), new Date().getMonth() + 1);
  const { ano: anoInicial, mes: mesInicial } = parseMes(inicial);
  ...
  const [rateHistory, setRateHist]= useState([]);
  const [contab, setContab]       = useState([]);
  const [obs, setObs]             = useState({});
  const [loading, setLoading]     = useState(true);

  const ms = mesStr(ano, mes);

  // Carregar dados estáticos uma vez
  useEffect(() => {
    if (!sb) return;
    Promise.all([
      ...
      sb.from('worker_valorhora_history').select('*'),
    ]).then(([w, l, c, r]) => {
      setWorkers(w.data || []);
      setLogs(l.data || []);
      setClients(c.data || []);
  ...
  // Carregar contabilidade_mensal e observações reactivamente ao mês
  useEffect(() => {
    if (!sb) return;
    Promise.all([
    ...
    // Real-time nas observações
    const channel = sb.channel(`pub_obs_${ms}`)
      .on('postgres_changes', { event: '*', schema: 'public', table: 'resumo_observacoes', filter: `mes=eq.${ms}` },
        ({ new: row, eventType }) => {
  ...
  function navMes(dir) {
    ...
    if (m < 1)  { m = 12; a--; }
    setMes(m); setAno(a);
    window.history.replaceState(null, '', `?mes=${mesStr(a, m)}`);
  }
  ...
  const rows = useMemo(() => {
    ...
    const logsDoMes = logs.filter(l => l.date?.startsWith(ms));

    return ativos.map(w => {
      const workerLogs = logsDoMes.filter(l => l.workerId === w.id || l.worker_id === w.id);
      ...
      const empresa    = [...new Set(workerLogs.map(l => l.clientId || l.client_id).filter(Boolean))]
      ...
      return {
        ...
        vencBase: eur2(parseFloat(w.vencimento_base)), subsAlimDias: String(subsAlimDias),
        subsAlimDia: eur2(parseFloat(w.subsidio_alimentacao_dia) || 0), subsAlimTotal: eur2(rc.subsAlimTotal),
        subsFerias: eur2(rc.subsFerias), subsNatal: eur2(rc.subsNatal),
        ajudas: eur2(rc.ajudaCustoNecessaria), baseIRS: eur2(rc.incidenciaIRS),
        taxaIRS: pct2(rc.taxaEfIRS), irsTotal: eur2(rc.irsTotal), ssTrab: eur2(rc.ssTrabalhador),
        totalAbonos: eur2(rc.totalAbonos), totalDesc: eur2(rc.totalDescontos),
        liquido: eur2(rc.liquido), ssPatronal: eur2(rc.ssPatronal), custoEmpresa: eur2(rc.custoEmpresa),
        brutoAlvo: eur2(brutoAlvo), observacao: obs[w.id] || '',
        _vencNum: parseFloat(w.vencimento_base) || 0,
        _subsAlimNum: rc.subsAlimTotal, _feriasNum: rc.subsFerias, _natalNum: rc.subsNatal,
        _ajudasNum: rc.ajudaCustoNecessaria, _irsNum: rc.irsTotal, _ssTrabNum: rc.ssTrabalhador,
        _abonosNum: rc.totalAbonos, _descNum: rc.totalDescontos, _liquidoNum: rc.liquido,
        _ssPatNum: rc.ssPatronal, _custoNum: rc.custoEmpresa, _brutoNum: brutoAlvo,
      };
    });
  }, [workers, logs, clients, rateHistory, contab, obs, ms, ano]);
  ...
  return (
    <div className="min-h-screen bg-slate-50">
      {/* Header */}
      <div className="bg-slate-900 text-white px-6 py-4 flex items-center justify-between gap-4 shadow-lg">
        <div>
        ...
        <div className="text-right">
          ...
          <p className="text-[10px] text-slate-500">{rows.length} trabalhadores</p>
        </div>
      </div>
