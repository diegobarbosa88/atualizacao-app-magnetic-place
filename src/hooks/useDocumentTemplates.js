...
export function useDocumentTemplates(supabase, { onError } = {}) {
  ...
  const handleApproveDocument = useCallback(async (doc, { companyName, companySignature, adminIp, stampStyle } = {}) => {
    ...
    try {
      ...
      const { data: tmpl, error: tErr } = await supabase
        .from('document_templates')
        .select('formato, stamp_admin_x, stamp_admin_y, stamp_admin_page, layout_settings')
        .eq('id', doc.template_id)
        .single();
      if (tErr) throw tErr;
      ...
      if (tmpl?.formato === 'html') {
        ...
        // na última). `generated_html` continua a gravar `finalHtml`
        // completo (timbre/rodapé inline), que é o que `openGeneratedPreview`
        // mostra depois — a extração/encolhimento de página só acontece
        // dentro de `generateHtmlDocumentPdf` (partilhada com o preview ao
        // vivo do painel "Ajustar Layout", ver templates/
        // TemplateLayoutSettingsModal.jsx), nunca no que fica gravado.
        const pdfBlob = await generateHtmlDocumentPdf(finalHtml, tmpl.layout_settings);
