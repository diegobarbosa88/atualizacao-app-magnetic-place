...
const ModoDocumentos = ({ workers }) => {
  ...
  return (
    <div className="space-y-4">
      ...
      {filtrados.length === 0 && (
        <p className="text-center text-[var(--slate-dim)] text-xs py-10">Nenhum documento encontrado.</p>
      )}
              ...
              {filtrados.map(doc => {
                ...
                return (
                  <tr key={doc.id} className="hover:bg-[var(--surface)] transition-colors">
                    ...
                    <td className="hidden sm:table-cell px-4 py-3 text-[var(--slate-dim)] whitespace-nowrap">
                      {doc.dataEmissao ? new Date(doc.dataEmissao).toLocaleDateString('pt-PT', { day: '2-digit', month: 'short', year: 'numeric' }) : '—'}
                    </td>
                    ...
                    <td className="px-4 py-3">
                      <div className="flex items-center justify-end gap-1">
                        <a href={doc.url} target="_blank" rel="noreferrer"
                          className="p-2 rounded-lg text-[var(--slate)] hover:bg-[var(--surface-dim)] hover:text-[var(--ink-mid)] transition-colors" title="Ver PDF">
                          <FileDown size={15} />
                        </a>
                        {doc.status === 'Rascunho' && (
                          <button onClick={() => enviar(doc)} disabled={enviandoId === doc.id}
                            className="p-2 rounded-lg text-[var(--slate)] hover:bg-emerald-50 hover:text-emerald-600 transition-colors disabled:opacity-40" title="Enviar para trabalhador">
                        ...
                        <button onClick={() => apagar(doc)} disabled={apagandoId === doc.id}
                          className="p-2 rounded-lg text-[var(--slate)] hover:bg-red-50 hover:text-red-500 transition-colors disabled:opacity-40" title="Apagar">
