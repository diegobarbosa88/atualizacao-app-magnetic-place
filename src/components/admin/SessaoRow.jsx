...
function SessaoRow({ sessao, onAlterarEstado, onApagarRegisto, onApagarSessao, onAdicionarACustos, onEnviarRecibos, onAtualizarAjudas, workers }) {
  ...
  return (
    <>
      {/* Linha de sessão */}
      <tr onClick={() => setAberto(o => !o)} className="cursor-pointer hover:bg-[var(--surface)] transition-colors select-none">
        <td className="px-4 py-3">
          <div className="flex items-center gap-2">
            <ChevronRight size={16} className={`text-[var(--slate)] transition-transform shrink-0 ${aberto ? 'rotate-90' : ''}`} />
            <div className="flex flex-col">
              <span className="text-sm font-black text-[var(--ink)] whitespace-nowrap leading-tight">
                {[...new Set(sessao.map(r => r.mes).filter(Boolean))].map(formatarMes).join(', ') || '—'}
              </span>
              <span className="text-[10px] text-[var(--slate-dim)] font-medium whitespace-nowrap">
        ...
        <td className="px-4 py-3">
          <span className="text-xs text-[var(--slate-dim)] font-medium">
          ...
          <div className="flex items-center justify-between gap-2">
            ...
            <div className="flex items-center gap-2">
              ...
              <button
                ...
                className="p-1.5 rounded-lg text-[var(--slate)] hover:bg-indigo-50 hover:text-indigo-600 transition-colors disabled:opacity-40"
              >
              ...
              <button
                ...
                className="p-1.5 rounded-lg text-[var(--slate)] hover:bg-emerald-50 hover:text-emerald-600 transition-colors disabled:opacity-40"
              >
              ...
              <button
                ...
                className="p-1.5 rounded-lg text-[var(--slate)] hover:bg-red-50 hover:text-red-600 transition-colors"
              >
                <Trash2 size={18} />
              </button>
            </div>
            ...
            <div className="rounded-xl border border-indigo-100 bg-indigo-50/50 p-3 space-y-2">
              ...
              <table className="w-full text-[10px]">
                ...
                <tbody className="divide-y divide-indigo-100">
                  {burstResultados.map(r => (
                    <tr key={r.nif}>
                      <td className="py-1.5 font-bold text-[var(--ink-mid)]">{r.worker?.name ?? r.nome ?? r.nif}</td>
                      ...
                      <td className="py-1.5 text-right text-[var(--ink-soft)]">{fmtV(r.parsed?.ssExtraido)}</td>
                      <td className="py-1.5 text-right text-[var(--ink-soft)]">{fmtV(r.parsed?.irsExtraido)}</td>
                      <td className="py-1.5 text-right text-emerald-600 font-bold">{fmtV(r.parsed?.ajudasCustoExtraido)}</td>
                      <td className="py-1.5 text-right text-[var(--ink-mid)] font-bold">{fmtV(r.parsed?.liquidoExtraido)}</td>
                    </tr>
                  ))}
                </tbody>
              ...
              <div className="flex items-center gap-2 pt-1">
                <button
                  ...
                  className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg bg-[var(--orange)] text-[var(--navy-solid)] text-xs font-bold disabled:opacity-40 hover:bg-[var(--orange-hover)] transition-colors"
                >
      ...
      {aberto && (
        <tr>
          <td colSpan={8} className="px-0 py-0">
            <div className="mx-4 mb-3 rounded-xl border border-[var(--border-soft)] overflow-hidden">
              <table className="w-full text-xs">
                <thead className="bg-[var(--surface)] border-b border-[var(--border-soft)]">
                  <tr>
                    ...
                    <th className="px-3 py-2 text-left text-[9px] font-black uppercase tracking-widest text-[var(--slate-dim)]">Mês</th>
                    <th className="px-3 py-2 text-right text-[9px] font-black uppercase tracking-widest text-[var(--slate-dim)]">Bruto</th>
                    <th className="px-3 py-2 text-right text-[9px] font-black uppercase tracking-widest text-[var(--slate-dim)]">Bruto PDF</th>
                    <th className="px-3 py-2 text-right text-[9px] font-black uppercase tracking-widest text-[var(--slate-dim)]">SS</th>
                    <th className="px-3 py-2 text-right text-[9px] font-black uppercase tracking-widest text-[var(--slate-dim)]">IRS</th>
                    <th className="px-3 py-2 text-right text-[9px] font-black uppercase tracking-widest text-[var(--slate-dim)]">Ajudas de Custo</th>
                ...
                <tbody className="divide-y divide-[var(--border-soft)]">
                  {sessao.map(r => {
                    ...
                    return (
                    <tr key={r.id} className={`hover:bg-[var(--surface)] transition-colors ${emFalta ? 'bg-amber-50/40' : ''}`} title={r.mensagem}>
                      <td className="px-3 py-2.5 font-bold text-[var(--ink)] max-w-[140px] truncate">
                      ...
                      <td className="px-3 py-2.5 text-[var(--ink-soft)] whitespace-nowrap">{r.mes ? formatarMes(r.mes) : '—'}</td>
                      <td className="px-3 py-2.5 text-right font-bold text-[var(--ink-mid)]">{r.bruto_plataforma != null ? `${Number(r.bruto_plataforma).toFixed(2)}€` : '—'}</td>
                      <td className="px-3 py-2.5 text-right font-bold text-indigo-700">{r.bruto_extraido != null ? `${Number(r.bruto_extraido).toFixed(2)}€` : '—'}</td>
                      <td className="px-3 py-2.5 text-right text-[var(--slate-dim)]">{r.ss_extraido != null ? `${Number(r.ss_extraido).toFixed(2)}€` : '—'}</td>
                      <td className="px-3 py-2.5 text-right text-[var(--slate-dim)]">{r.irs_extraido != null ? `${Number(r.irs_extraido).toFixed(2)}€` : '—'}</td>
                      ...
                      <td className="px-3 py-2.5">
                        <div className="flex items-center justify-center gap-2">
                          ...
                          <button
                            ...
                            className="ml-1 p-1 rounded text-[var(--slate)] hover:bg-red-50 hover:text-red-600 transition-colors"
                          >
                            <Trash2 size={11} />
                          </button>
                        </div>
