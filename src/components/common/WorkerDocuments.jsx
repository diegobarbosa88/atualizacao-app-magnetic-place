  ...
  return (
    <div className="space-y-4 mb-6">
      ...
      {!pendingOnly && (
        <div className="flex gap-1 bg-slate-100 p-1 rounded-2xl">
          <button
            ...
            className={`flex-1 py-2 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all ${activeTab === 'pendentes' ? 'bg-white text-indigo-600 shadow-sm' : 'text-slate-400'}`}
          >
            Pendentes{pendentes.length > 0 && ` (${pendentes.length})`}
          </button>
          <button
            onClick={() => setActiveTab('historico')}
            className={`flex-1 py-2 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all flex items-center justify-center gap-1 ${activeTab === 'historico' ? 'bg-white text-indigo-600 shadow-sm' : 'text-slate-400'}`}
          >
            Histórico{historico.length > 0 && ` (${historico.length})`}
            {activeTab === 'historico' && (
              <button
                onClick={e => { e.stopPropagation(); setShowFilters(!showFilters); }}
                className={`ml-1 p-0.5 rounded transition-colors ${showFilters ? 'text-indigo-600' : 'text-slate-400'}`}
              >
                <Filter size={11} />
              </button>
            )}
      ...
      {!pendingOnly && activeTab === 'historico' && showFilters && (
        <div className="bg-white border border-slate-100 rounded-2xl px-4 py-3 flex flex-wrap gap-3 items-center">
          <div className="flex items-center gap-2">
            <label className="text-[9px] font-black text-slate-400 uppercase">Tipo</label>
            <select value={filterType} onChange={e => setFilterType(e.target.value)}
              className="px-2 py-1 rounded-lg bg-slate-50 border border-slate-200 text-xs font-bold outline-none">
        ...
        <div className="space-y-2">
          {docList.map(doc => {
            ...
            return (
              <div
                key={doc.id}
                className={`bg-white border rounded-2xl px-4 py-3.5 flex items-center gap-3 ${pending ? 'border-amber-100' : 'border-slate-100'}`}
              >
                {/* Ícone */}
                <div className={`w-9 h-9 rounded-xl flex items-center justify-center shrink-0 ${pending ? 'bg-amber-50' : 'bg-emerald-50'}`}>
                  {isTemplate
                    ? <FileSignature size={16} className={pending ? 'text-amber-500' : 'text-emerald-500'} />
                    : <FileText size={16} className={pending ? 'text-amber-500' : 'text-emerald-500'} />}
                </div>
                ...
                {pending ? (
                  <button
                    onClick={() => openDoc(doc)}
                    className="shrink-0 flex items-center gap-1.5 px-3 py-2 bg-indigo-600 text-white rounded-xl text-[10px] font-black uppercase tracking-wide hover:bg-indigo-700 transition-all active:scale-95"
                  >
                  ...
                  <a
                    ...
                    className="shrink-0 p-2 text-slate-300 hover:text-emerald-600 hover:bg-emerald-50 rounded-xl transition-all"
                  >
                    <Download size={15} />
                  </a>
                )}
