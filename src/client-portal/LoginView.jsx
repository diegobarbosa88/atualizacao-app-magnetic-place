...
export default function LoginView({ t, lang, changeLang, loginNif, setLoginNif, loginEmail, setLoginEmail, loginError, handleLogin, submitting, systemSettings }) {
    return (
        <div className="min-h-screen bg-white flex flex-col lg:flex-row overflow-hidden">
            ...
            <div className="lg:w-3/5 relative flex flex-col justify-between p-8 md:p-14 overflow-hidden bg-[#122741] min-h-[45vh] lg:min-h-screen">

                <div className="absolute inset-0 bg-gradient-to-br from-[#122741] via-[#1B3A57] to-[#20415F]" />
                ...
                <div className="absolute top-[-10%] right-[-5%] w-[500px] h-[500px] bg-[#EB8D00]/20 rounded-full blur-[120px]" />
                <div className="absolute bottom-[-15%] left-[-5%] w-[450px] h-[450px] bg-[#20415F]/40 rounded-full blur-[100px]" />
                ...
                <div className="relative z-10 my-auto py-10">
                    <div className="inline-flex items-center gap-2 bg-white/10 border border-white/20 rounded-full px-4 py-2 mb-8">
                        <span className="w-2 h-2 bg-[#EB8D00] rounded-full animate-pulse" />
                    ...
                    <h1 className="text-6xl md:text-7xl lg:text-8xl font-black uppercase leading-[0.9] tracking-tighter">
                        ...
                        <span className="text-transparent bg-clip-text bg-gradient-to-r from-[#EB8D00] via-[#FBCB77] to-[#EB8D00]">{t('panel_title_2').split('\n').map((line, i) => <React.Fragment key={i}>{line}{i < t('panel_title_2').split('\n').length - 1 && <br />}</React.Fragment>)}</span>
                    </h1>
                ...
                <div className="w-full max-w-sm">
                    ...
                    <div className="flex justify-end mb-6">
                        <div className="flex rounded-xl overflow-hidden border border-slate-200 shadow-inner">
                            <button onClick={() => changeLang('pt')} title="Português" className={`flex items-center gap-1.5 px-3 py-2 transition-all ${lang === 'pt' ? 'bg-[#FDF3E4]' : 'bg-white opacity-50 hover:opacity-80'}`}>
                            ...
                            <button onClick={() => changeLang('es')} title="Español" className={`flex items-center gap-1.5 px-3 py-2 transition-all border-l border-slate-200 ${lang === 'es' ? 'bg-[#FDF3E4]' : 'bg-white opacity-50 hover:opacity-80'}`}>
                    ...
                    <div className="mb-8">
                        <p className={`${SCALE.text.statLabel} text-[#8a4a00] mb-2`}>{t('welcome')}</p>
                    ...
                    <form className="space-y-4" onSubmit={e => { e.preventDefault(); handleLogin(); }}>
                        <div>
                            ...
                            <input type="email" value={loginEmail} onChange={e => setLoginEmail(e.target.value)} placeholder={t('email_placeholder')} autoComplete="email"
                                className="w-full px-5 py-4 bg-slate-50 border border-slate-200 rounded-2xl text-slate-800 placeholder-slate-300 font-bold text-sm focus:outline-none focus:border-[#EB8D00] focus:bg-white focus:ring-2 focus:ring-[#FDF3E4] transition-all" />
                        </div>
                        <div>
                        ...
                        <button type="submit" disabled={submitting}
                            className="w-full py-4 bg-[#EB8D00] hover:bg-[#F59B1C] text-[#1B3A57] rounded-2xl font-black text-sm uppercase tracking-[0.15em] transition-all shadow-lg shadow-[#FBE7C6] active:scale-95 disabled:opacity-50 disabled:cursor-not-allowed">
