...
must_haves:
  truths:
    - "Componente ReconciliacaoAdmin.jsx criado com zona de upload drag & drop"
    - "Após processamento, aparecem sub-tabs: Reconciliados (N) | Órfãos Banco (N) | Órfãos Sistema (N)"
    - "Sub-tab Reconciliados mostra botão Confirmar Pagamento por linha"
    - "Confirmar Pagamento chama supabase.from('faturas').update({ status: 'PAGO' }).eq('id', fatura.id)"
    - "Secção Histórico colapsável mostra runs anteriores com data, ficheiro e contagens"
    - "Formulário de inserção manual de fatura com campos: tipo, valor, data_documento, descricao, entidade, fonte"
    - "Design system consistente: rounded-2xl, indigo para ações, emerald para matches, amber para ambíguos, rose para órfãos"
  artifacts:
    - path: "src/features/admin/ReconciliacaoAdmin.jsx"
      provides: "UI completo de reconciliação bancária"
      contains: "ReconciliacaoAdmin\|Confirmar Pagamento\|orphan_bank\|orphan_system"
  key_links:
    - from: "ReconciliacaoAdmin.jsx"
      to: "/api/reconciliacao/upload"
      via: "fetch com FormData"
      pattern: "fetch.*reconciliacao/upload\|FormData"
    - from: "botão Confirmar Pagamento"
      to: "supabase.from('faturas').update"
      via: "useApp().supabase"
      pattern: "supabase.*faturas.*update\|status.*PAGO"
    - from: "Histórico"
      to: "supabase.from('reconciliation_runs').select()"
      via: "useEffect ao montar"
      pattern: "reconciliation_runs.*select"
---

<objective>
Criar o componente `ReconciliacaoAdmin.jsx` com a UI completa: upload drag & drop, sub-tabs de resultados, confirmação de pagamento por linha, formulário de inserção manual de fatura, e histórico de runs anteriores.

Purpose: Interface visual que o admin usa para importar extratos e confirmar pagamentos.
Output: Componente funcional em `src/features/admin/ReconciliacaoAdmin.jsx` pronto para ser ligado ao AdminDashboard no Plan 05.
</objective>
...
<interfaces>
<!-- useApp hook (AppContext.jsx): -->
const { supabase } = useApp();
// supabase é o cliente Supabase do frontend (anon key)
// saveToDb(table, id, data) — upsert genérico, mas para faturas usar supabase directo

<!-- Design system (TeamManager.jsx padrão): -->
// rounded-[2.5rem] para cards principais
// rounded-2xl para elementos secundários
// bg-indigo-600 hover:bg-indigo-700 para botões de acção primária
// bg-emerald-50 text-emerald-700 para matches/sucesso
// bg-amber-50 text-amber-700 para ambíguos/aviso
// bg-rose-50 text-rose-700 para órfãos/erro
// text-[10px] font-black uppercase tracking-widest para labels de secção
// shadow-sm border border-slate-100 para cards

<!-- Resposta da API (Plan 03): -->
// {
//   ok: true,
//   run_id: string,
//   filename: string,
...
//   orphan_bank_count: number,
//   orphan_system_count: number,
//   matched: Array<{ transacao, fatura, rule }>,
//   orphan_bank: Array<{ transacao, reason, candidates? }>,
//   orphan_system: Array<{ fatura, reason }>,
// }
...
<task type="auto">
  <name>Task 1: Criar ReconciliacaoAdmin.jsx — Upload + Sub-tabs de Resultados</name>
  <read_first>
    - src/features/admin/DocumentsAdmin.jsx (padrão de upload visual e sub-tabs)
    - src/features/admin/FaturasAdmin.jsx (padrão de layout e supabase client)
    - .planning/phases/18-reconciliacao-bancaria/18-CONTEXT.md §D-17, D-18 (layout e design)
    - .planning/phases/18-reconciliacao-bancaria/18-CONTEXT.md §D-08, D-09, D-10 (workflow confirmação)
  </read_first>
  <files>src/features/admin/ReconciliacaoAdmin.jsx</files>
  <action>
Criar `src/features/admin/ReconciliacaoAdmin.jsx` com a estrutura completa.
...
export default function ReconciliacaoAdmin() {
  ...
  const processar = async () => {
    ...
    try {
      ...
      if (!res.ok) {
        setErro(data.error || 'Erro ao processar ficheiro.');
        if (data.detected) setErro(`${data.error} Colunas detectadas: ${data.detected.join(', ')}`);
        return;
      }
      setResultado(data);
  ...
  const confirmarPagamento = async (faturaId) => {
    ...
    try {
      ...
      // Actualizar estado local — remover da lista de matched
      setResultado(prev => ({
        ...prev,
              ...
              {(displayData.orphan_bank || []).map((item, i) => (
                <div key={i} className={`rounded-2xl p-4 ${item.reason === 'ambiguous' ? 'bg-amber-50' : 'bg-rose-50'}`}>
                  <div className="flex items-center gap-2 flex-wrap mb-1">
                    ...
                    <span className="text-sm font-bold text-slate-700">€{item.transacao.valor.toFixed(2)}</span>
...
```
  ...
  <verify>
    <automated>grep -c "Confirmar Pagamento\|orphan_bank\|orphan_system\|reconciliation_runs\|FormData" "src/features/admin/ReconciliacaoAdmin.jsx"</automated>
  </verify>
  <acceptance_criteria>
    - grep "Confirmar Pagamento" retorna match no JSX (botão visível)
    - grep "confirmarPagamento" retorna match (função implementada)
    - grep "supabase.*from.*faturas.*update" ou "from('faturas').update" retorna match
    - grep "status.*PAGO\|PAGO.*status" retorna match (update correcto)
    - grep "onDragOver\|onDragLeave\|onDrop" todos retornam match (drag & drop implementado)
    - grep "reconciliation_runs.*select\|select.*reconciliation_runs" retorna match (histórico carregado)
    - grep "orphan_bank" retorna pelo menos 5 ocorrências
    - grep "orphan_system" retorna pelo menos 5 ocorrências
    - grep "Inserir Fatura Manual" retorna match (formulário presente)
    - grep "gmail_message_id.*manual\|manual.*gmail_message_id" retorna match (campo NOT NULL tratado)
    - grep "setRunSelecionado" retorna match (navegação para run do histórico)
    - Ficheiro tem mais de 200 linhas (componente completo)
  </acceptance_criteria>
  <done>ReconciliacaoAdmin.jsx criado com upload drag & drop, sub-tabs de resultados com dados reais, botão Confirmar Pagamento individual, formulário de fatura manual, e histórico colapsável. Pronto para ligação no AdminDashboard (Plan 05).</done>
</task>
...
|----------|-------------|
| ReconciliacaoAdmin → Supabase faturas | Update de status diretamente do frontend via anon key |

## STRIDE Threat Register

| Threat ID | Category | Component | Disposition | Mitigation Plan |
|-----------|----------|-----------|-------------|-----------------|
| T-18-10 | Tampering | supabase.update({ status: 'PAGO' }) no frontend | accept | Admin único autenticado; RLS garante que apenas sessão autenticada pode escrever; mudança não é reversível mas é auditável via campo updated_at |
| T-18-11 | Spoofing | Histórico de runs (read-only) | accept | D-12 define explicitamente que não se pode refazer confirmações a partir do histórico — UI de histórico é read-only |
</threat_model>

<verification>
```bash
# Verificar estrutura do componente:
grep -n "export default\|confirmarPagamento\|processar\|carregarHistorico\|guardarFatura" src/features/admin/ReconciliacaoAdmin.jsx
# Deve mostrar 5 funções definidas

# Verificar imports lucide:
grep "Landmark\|ArrowLeftRight\|CheckCircle" src/features/admin/ReconciliacaoAdmin.jsx
# Deve retornar match
```
</verification>

<success_criteria>
- ReconciliacaoAdmin.jsx criado sem erros de sintaxe
- Drag & drop com validação de formato (CSV/OFX aceites, PDF rejeitado com mensagem PT)
- Sub-tabs aparecem após processamento com contagens correctas
- Botão "Confirmar Pagamento" visível na sub-tab Reconciliados (não no histórico)
- Formulário manual com campos tipo, valor, data_documento, descricao, entidade
- Histórico colapsável carregado do Supabase ao montar
- Cores do design system: emerald para matched, amber para ambiguous, rose para orphans
</success_criteria>

<output>
Após conclusão, criar `.planning/phases/18-reconciliacao-bancaria/18-04-SUMMARY.md`
</output>
