...
- Name: 03-painel-admin\edicao-correcoes.spec.js >> Edição de Correções (Admin) >> Deve mostrar notificação de correção na lista
- Location: tests\e2e\03-painel-admin\edicao-correcoes.spec.js:93:3
...
```ts
  ...
  4   | 
  5   | test.describe('Edição de Correções (Admin)', () => {
  6   |   let mockSupabase;
  7   |   let testWorker;
  8   |   let testClient;
  9   | 
  10  |   test.beforeEach(async ({ page }) => {
  11  |     mockSupabase = resetMockSupabase();
  12  | 
  13  |     testWorker = createWorker({
  14  |       id: 'w_edit_test',
